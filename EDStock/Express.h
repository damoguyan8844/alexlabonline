...
extern	SLH_DATA	slh_data_array[SLH_MAX+1];
	...
	CSPString	AfxGetVariantName( UINT nVariantID, BOOL bWithParameter );	// 根据股票列表项ID得到名称，bWithParameter表示名称中是否含有参数
	BOOL		AfxGetVariantNameArray( CSPStringArray & astr, BOOL bWithParameter );	// 得到所有股票列表项的名称
	UINT		AfxGetVariantID( CSPString strName, BOOL bWithParameter );	// 根据股票列表项名称得到ID
	CSPString	AfxGetVariantVarName( int nVariantID );		// 根据股票列表项ID得到使用在表达式中的变量名称
	CSPString	AfxGetVariantVarName( CSPString strName, BOOL bWithParameter );		// 根据股票列表项名称得到使用在表达式中的变量名称
	...
	COLORREF	AfxGetVariantColor( UINT nVariantID, CStockInfo &info );	// 得到某只股票的相应值的显示颜色
