/*
 * $Id: MetaLinkQuerySocket.cpp 13559 2009-06-09 06:57:05Z huby $
 * 
...
CStringA CMetaLinkQuerySocket::GetUrlPath()
{
	...
	try
	{
		...
		{			
			...
			if(uSize == 0)
				return "";
			CStringA strPartFileURL = (CStringA)m_pMgr->m_pAssocPartFile->GetPartFileURL();
			...
			{				
				...
				strHash = md4str(md4.GetHash());
				strHash.MakeLower();
				m_strUrlPath.Format("/app/emule/metalink/finish/http/%s/%s", strHash, CStringA(EncodeUrlUtf8(CString(strPartFileURL))));
			} 
			else // ed2k任务
			{
				char szSize[1024];
