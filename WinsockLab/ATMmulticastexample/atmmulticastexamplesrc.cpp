...
void Server(SOCKET s, WSAPROTOCOL_INFO *lpSocketProtocol)
{
    ...
    if (WSAAddressToString((LPSOCKADDR)&atmroot, sizeof(atmroot), lpSocketProtocol, (LPWSTR)szAddr,
		&dwAddrLen))
    {
...
void Client(SOCKET s, WSAPROTOCOL_INFO *lpSocketProtocol)
{
    ...
    if (WSAAddressToString((LPSOCKADDR)&atm_leaf, sizeof(atm_leaf), lpSocketProtocol, (LPWSTR)szAddr,
        &dwAddrLen))
    {
        printf("WSAAddressToString() failed with error code %d\n", WSAGetLastError());
    }
