    ...
    Else
        ...
        With m_oAPIHooker2
            ...
            If GetModuleHandle("vba6.dll") = 0 Then
                ...
                lHookIndex = .Hook("msvbvm60.dll", "user32.dll", "MessageBoxIndirectA")
            Else
...
Private Sub Command4_Click()
    ...
    Else
        ...
        With m_oApiHooker
            ...
            .SetTargetTo pid
            lHookIndex = .Hook("notepad.exe", "comdlg32.dll", "GetOpenFileNameW")
            lHookIndex2 = .Hook("notepad.exe", "comdlg32.dll", "ChooseFontW")
        End With
    End If
End Sub
...
Private Sub m_oAPIHooker2_BeforeAction(ByVal sAPIName As String, ByVal lpArgs As Long, fIntercept As Boolean, lArgCount As Long, ByVal lReturnAddress As Long)
    AddMsg "即将调用函数 " & sAPIName & " ,参数堆栈地址为：&H" & Hex(lpArgs)
...
Private Sub m_oAPIHooker2_AfterAction(ByVal sAPIName As String, ByVal fIntercepted As Boolean, lRetValue As Long)
    AddMsg "函数 " & sAPIName & IIf(fIntercepted, "被阻止", " 调用完毕,返回参数为：&H" & Hex(lRetValue))
