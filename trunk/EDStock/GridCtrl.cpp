...
BOOL CGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, int nKeyCol, BOOL bAscending,
                          LPARAM data /* = 0 */)
{
    SetSortColumn(nCol);
    SetSortAscending(bAscending);
    ResetSelectedRange();
    SetFocusCell(-1, -1);
    return SortItems(pfnCompare, nCol, nKeyCol, bAscending, data, GetFixedRowCount(), -1);
}
...
BOOL CGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, int nKeyCol, BOOL bAscending, LPARAM data,
                          int low, int high)
{
	nCol = nKeyCol;
    ...
    while (lo <= hi)
    {
        ...
        if (bAscending)
            ...
            while (lo < high && pfnCompare(pMidCell, (LPARAM)GetCell(lo, nCol), data) > 0)
				++lo;
		...
		if (bAscending)
			...
			while (hi > low && pfnCompare(pMidCell, (LPARAM)GetCell(hi, nCol), data) < 0)
				--hi;
    ...
    if (low < hi)
        SortItems(pfnCompare, nCol, nKeyCol, bAscending, data, low, hi);
    
    // If the left index has not reached the right side of array
    // must now sort the right partition.
    if (lo < high)
        SortItems(pfnCompare, nCol, nKeyCol, bAscending, data, lo, high);
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    {
        SetFocusCell(-1,-1);
        SetFocusCell(max(m_LeftClickDownCell.row, GetFixedRowCount()),
			max(m_LeftClickDownCell.col, GetFixedColumnCount()));
