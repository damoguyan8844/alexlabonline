...
// Description: Free the buffer object. This adds the object to the free lookaside list.
void FreeBufferObj(BUFFER_OBJ *obj)
{
    EnterCriticalSection(&gBufferListCs);
...
DWORD WINAPI CompletionThread(LPVOID lpParam)
{
    ...
    {
        error = NO_ERROR;
        rc = GetQueuedCompletionStatus(CompletionPort, &BytesTransfered, (PULONG_PTR)&Key,
            ...
            if (interval == 36)
            {
                ...
                while (listenobj)
                {
                    ...
                    while (acceptobj)
                    {
                        ...
                        rc = getsockopt( acceptobj->sclient, SOL_SOCKET, SO_CONNECT_TIME,
							(char *)&optval, &optlen);
                        if (rc == SOCKET_ERROR)
                        {
