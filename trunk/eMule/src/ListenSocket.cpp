...
void CClientReqSocket::SetConState( SocketState val )
{
	...
	case SS_Half:
		{
			if(m_bUseNat)
				theApp.listensocket->m_nHalfOpenOfL2L--;
			else
...
bool CClientReqSocket::CheckTimeOut()
{
	...
	if (::GetTickCount() - timeout_timer > uTimeout){
		...
		if(theApp.natthread)
		{
			...
			uint32 timeout_nat=CONNECTION_TIMEOUT;	//Enig123::trivial change - less warning
...
void CClientReqSocket::Safe_Delete()
{
	...
	if( theApp.natthread && m_bUseNat )
	{
		if( client && client->GetUserHash() )
			theApp.natthread->RemoveStrategy(client->GetUserHash());
		theApp.natthread->RemoveSocket(this,m_bDisconnectAll,client? client->GetUserHash() : NULL);
	}
	...
	try
	{
		try
		{
			...
			switch(opcode)
			{
				...
				//Enig123::official new opcode handle ->>
