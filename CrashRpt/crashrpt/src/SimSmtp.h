// Smtp.h: interface for the CSimSmtp class.
//
...
// The CSimSmtp::SendCmd() function will send blocks no larger than this value
...
#ifndef MESSAGE_ENCODING_PLAIN
#define MESSAGE_ENCODING_PLAIN _T("text/plain")
#endif

#ifndef MESSAGE_ENCODING_HTML
#define MESSAGE_ENCODING_HTML _T("text/html")
#endif
...
{
public: // Constructors
  CSmtpMessageBody(LPCTSTR pszBody = NULL, LPCTSTR pszEncoding = MESSAGE_ENCODING_PLAIN, LPCTSTR pszCharset = MESSAGE_CHARSET, EncodingEnum encode = encodeGuess);
...
// This class represents a single message that can be sent via CSimSmtp
class CSmtpMessage
{
public: // Constructors
  CSmtpMessage();
...
public: // These represent the primary public functionality of this class
	BOOL Connect(LPTSTR pszServer,WORD nPort);
	int  SendMessage(CSmtpMessage& msg);
	int  SendMessage(CSmtpAddress& addrFrom, CSmtpAddress& addrTo, LPCTSTR pszSubject, LPCTSTR pszMessage, LPVOID pvAttachments = NULL, DWORD dwAttachmentCount = 0);
	int  SendMessage(LPCTSTR pszAddrFrom, LPCTSTR pszAddrTo, LPCTSTR pszSubject, LPCTSTR pszMessage, LPVOID pvAttachments = NULL, DWORD dwAttachmentCount = 0);
	void Close();
